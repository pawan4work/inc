...
var dbRoute = 'mongodb://dbusr_rw:dbusr_passwd@127.0.0.1:27017/p_incdb';

/*------------Team 1 ------------------*/
exports.getFirstTeam = function(req, res) {
    MongoClient.connect(dbRoute, function(err, db) {
        if(err) throw err;
        db.collection('incdetails').find({'owning_group':'owner_support_1'}).toArray(function(err, docs) {
            if (docs) {
...
exports.getSecondTeam = function(req, res) {
    MongoClient.connect(dbRoute, function(err, db) {
        if(err) throw err;
        db.collection('incdetails').find({'owning_group':'owner_support_2'}).toArray(function(err, docs) {
            if (docs) {
                return res.json({'success': true,'message': '', data:docs});
            } else {
                return res.json({'success': false,'message': 'Oops! Somthing went wrong.'});
